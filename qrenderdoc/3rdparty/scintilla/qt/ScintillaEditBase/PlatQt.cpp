...
#if QT_VERSION >= QT_VERSION_CHECK(6, 0, 0)
	QString su = (codec && codec->isValid()) ? codec->decode(QByteArrayView(s, len)) : QString::fromUtf8(s, len);
#else
...
XYPOSITION SurfaceImpl::WidthText(Font &font, const char *s, int len)
{
	...
	SetCodec(font);
#if QT_VERSION >= QT_VERSION_CHECK(6, 0, 0)
	QString string = (codec && codec->isValid()) ? codec->decode(QByteArrayView(s, len)) : QString::fromUtf8(s, len);
...
#if QT_VERSION >= QT_VERSION_CHECK(6, 0, 0)
	QScreen *screen = window(wid)->screen();
